...
if (Loader::includeModule('collected.activelife')){

    if (!empty($arParams['OFFER_TREE_COLOR_PROPS'])) {
        $c = Option::get('collected.activelife', 'color_table_count', 0);
        $arrColors = array();
        for ($i = 0; $i < $c; $i++) {
            $name = Option::get('collected.activelife', 'color_table_name_'.$i, '');
            $rgb = Option::get('collected.activelife', 'color_table_rgb_'.$i, '');
            if ($name != '' && $rgb != '') {
...
$arResult = RSDevLibResultModifier::CatalogSmartFilter($arResult);
    ...
    foreach($arResult['ITEMS'] as $key => $arItem){
        if (
            ...
            if ('N' == $arItem['PROPERTY_TYPE'] || isset($arItem['PRICE'])) {
                $minCalculated = RSDevLibFilterExtension::RoundCustom($arItem['VALUES']['MIN']['VALUE'], RSDevLibFilterExtension::GetTo4round($arItem['VALUES']['MIN']['VALUE']), 'simple');
                $maxCalculated = RSDevLibFilterExtension::RoundCustom($arItem['VALUES']['MAX']['VALUE'], RSDevLibFilterExtension::GetTo4round($arItem['VALUES']['MAX']['VALUE']), 'simple');
